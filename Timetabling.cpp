...
long double Timetabling::eventInPeriodNumber(int pi, int n2)
{
	...
	for (int i = 0; i < 5; i++)
	{
		if (((pi > i) && (event[n2].ei == pi - i - 1)) || ((pi < numSlots - i - 1) && (event[n2].ei == pi + i + 1)))
			return i;
	}
	return -1;
}
...
void Timetabling::readTimetable(const char *name)
{
	int e, slot;
	FILE *tt;
	char nameT[32];
...
void Timetabling::evaluate(const char *name)
{
	readSlot(name);
	readEvents(name);
	readStudents(name);
...
void Timetabling::readSlot(const char *name)
{
	FILE *fp;
	char nameS[32];
...
void Timetabling::readEvents(const char *name)
{
	int i, j, e, enroll;
	FILE *fp;
	char nameE[32];
...
void Timetabling::readStudents(const char *name)
{
	FILE *fp;
	int i, e;
